Este proyecto constituye el backend y el núcleo de inteligencia artificial para un sistema de triaje dermatológico. Utiliza redes neuronales convolucionales (CNN) optimizadas para dispositivos móviles con el objetivo de clasificar lesiones cutáneas en dos categorías: Benignas y Malignas (Melanoma).
...
El repositorio está organizado de la siguiente manera para separar la lógica de entrenamiento, la inferencia y la conversión del modelo:
...
├── convert_ptl.py              # Script de serialización a TorchScript (Mobile)
...
Gestión del Desbalance: Se identificó un desbalance significativo entre muestras benignas y malignas, lo cual motivó la implementación de estrategias de ponderación en la función de pérdida.
...
Backbone: Se utilizó MobileNetV2 pre-entrenada en ImageNet como extractor de características. Esta red es ideal para entornos móviles debido a su uso de Depthwise Separable Convolutions, que reducen drásticamente el coste computacional.
...
Weighted Cross Entropy Loss: Para evitar que el modelo sesgara sus predicciones hacia la clase mayoritaria (Benigno), se implementó una función de pérdida ponderada. Se asignó un peso mayor a la clase "Maligno" (ratio aprox. 1:8). Esto penaliza severamente los Falsos Negativos, priorizando la sensibilidad del sistema (seguridad médica).
...
El archivo main.py expone el modelo a través de una API REST construida con FastAPI.
...
Iniciar el servidor: Utilice Uvicorn (servidor ASGI) para lanzar la aplicación. Ejecute el siguiente comando en la terminal:
...
Acceder a la Interfaz de Pruebas: FastAPI genera automáticamente documentación interactiva basada en el estándar OpenAPI. Abra su navegador y vaya a:
...
Macro/Primer Plano: La lesión debe ocupar la parte central de la imagen. Evite fotos tomadas a más de 10-15 cm de distancia.

Iluminación: Use luz natural o flash. Las sombras oscuras pueden ser interpretadas erróneamente como pigmentación maligna.

Enfoque: La textura de la piel debe verse nítida. Si la imagen está borrosa, el modelo detectará bordes difusos, lo cual es un signo clínico de malignidad, provocando un Falso Positivo.

Obstrucciones (Importante): Evite la presencia de vello grueso sobre la lesión. El modelo puede confundir los pelos oscuros con el "retículo pigmentado atípico" (una estructura vascular asociada al melanoma). Si es posible, aparte el vello o tome la foto desde un ángulo limpio.
